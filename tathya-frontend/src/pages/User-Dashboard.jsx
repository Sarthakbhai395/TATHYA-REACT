...
import { getNotifications } from '../services/notificationService'; // Add this import
...
const UserDashboard = () => {
  ...
  const fetchModerators = async () => {
    try {
      // For regular users, we should fetch all users with role 'moderator'
      const response = await axios.get(`${API_BASE}/auth/moderators`, {
        headers: getAuthHeaders()
      });
      
      console.log('Moderators API response:', response.data);
      ...
      if (Array.isArray(data)) {
        ...
        // Set first moderator as default selected if none selected
        if (data.length > 0 && !selectedModerator) {
          setSelectedModerator(data[0]);
        }
      } else {
  ...
  // Fetch messages between current user and selected moderator
  const fetchMessagesWithModerator = async (moderatorId) => {
    try {
      // Get all user messages
      const allMessages = await getUserMessages();
      
      // Filter messages between current user and selected moderator
      const filteredMessages = allMessages.filter(msg => 
        (msg.from._id === moderatorId && msg.to._id === JSON.parse(localStorage.getItem('tathya_user'))._id) ||
        (msg.from._id === JSON.parse(localStorage.getItem('tathya_user'))._id && msg.to._id === moderatorId)
      );
      
      setModeratorMessages(filteredMessages);
    } catch (error) {
      console.error('Failed to fetch messages with moderator:', error);
      setModeratorMessages([]);
    }
  };
  ...
  useEffect(() => {
    ...
    const handleReceiveMessage = (message) => {
      console.log('User Dashboard: Received message via socket:', message);
      
      // Add new message to the messages list if it's from the selected moderator
      if (selectedModerator && message.from._id === selectedModerator._id) {
    ...
    const handleMessageSent = (message) => {
      console.log('User Dashboard: Message sent confirmation via socket:', message);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        try {
          ...
          console.log('Failed to load moderators:', moderatorErr.message);
        }
        ...
        try {
          ...
          console.log('Failed to load notification count:', notificationErr.message);
        }
    ...
    const notificationInterval = setInterval(async () => {
      try {
        ...
        console.log('Failed to refresh notification count:', error.message);
      }
    }, 30000); // 30 seconds
  ...
  // Fetch messages when selected moderator changes
  useEffect(() => {
    if (selectedModerator && selectedModerator._id) {
      fetchMessagesWithModerator(selectedModerator._id);
    }
  }, [selectedModerator]);
  ...
  const handleSendMessage = async () => {
    if (!messageInput.trim() || !selectedModerator || sendingMessage) return;
    ...
    try {
      ...
      const tempMessage = {
        ...
        from: { _id: JSON.parse(localStorage.getItem('tathya_user'))._id },
        to: { _id: selectedModerator._id },
      ...
      if (!socketSuccess) {
        ...
        await sendMessage(selectedModerator._id, messageInput);
        // Remove temporary message and add the real one
        setModeratorMessages(prevMessages => 
          prevMessages.filter(msg => msg._id !== tempMessage._id)
        );
      }
      // If socketSuccess is true, the message will be handled by the socket event listeners
    } catch (error) {
  ...
  // Fetch contact queries when active menu changes to 'contact-queries'
  useEffect(() => {
    const fetchContactQueries = async () => {
      try {
        setLoadingQueries(true);
        console.log('Fetching contact queries...');
        const queries = await dashboardService.getUserContactQueries();
        console.log('Fetched contact queries:', queries);
    ...
    // Only fetch queries when on the contact queries section
    if (activeMenu === 'contact-queries') {
      fetchContactQueries();
    }
  }, [activeMenu]);
  ...
  const handleCloseDocModal = () => {
   ...
   const handlePreviewDocument = async (e, docId) => {
    ...
    if (doc.file) {
     ...
     try {
      // For PDFs, we want to view them inline, not download them
      const action = doc.type.toLowerCase() === 'pdf' ? 'view' : 'view';
      const res = await fetch(`${API_BASE}/documents/${docId}/download?action=${action}`, {
        method: 'GET',
        headers: {
          ...getAuthHeaders(),
        },
  ...
  const menuItems = [
    ...
    { id: 'contact-queries', icon: 'fa-envelope', label: 'My Queries' },
        ...
        <header className="dashboard-header bg-white shadow-sm py-4 px-6 flex justify-between items-center sticky top-0 z-10">
          <div className="notification-bell relative">
            <button 
              ...
              className="relative p-3 rounded-full bg-gray-100 hover:bg-blue-100 transition-all duration-300 transform hover:scale-110"
            >
              <i className="fas fa-bell text-xl text-gray-700 hover:text-blue-600"></i>
              {notificationCount > 0 && (
                <motion.span 
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
                  className="absolute top-0 right-0 bg-red-500 text-white text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center animate-pulse"
                >
                  {notificationCount}
                </motion.span>
              )}
        ...
        <div className="dashboard-content-area p-6">
          ...
          {activeMenu === 'moderator-messages' && (
            <div className="moderator-messages-section animate-fade-in">
              <h2 className="text-2xl font-bold text-gray-800 mb-6">Chat with Moderator</h2>
              <div className="bg-white p-6 rounded-lg shadow-md">
                {moderators.length > 0 ? (
                  <div className="flex flex-col md:flex-row gap-6">
                    {/* Moderators List */}
                    <div className="md:w-1/3">
                      <h3 className="text-lg font-semibold text-gray-800 mb-3">Select Moderator</h3>
                      <div className="space-y-2 max-h-96 overflow-y-auto">
                        {moderators.map((moderator) => (
                          <div
                            key={moderator._id}
                            className={`p-3 rounded-lg cursor-pointer transition-all duration-200 ${
                              selectedModerator && selectedModerator._id === moderator._id
                          ...
                          >
                            <div className="flex items-center">
                              <img
                                src={moderator.avatar || generateUniqueAvatar(moderator)}
                                alt={moderator.fullName}
                                className="w-10 h-10 rounded-full object-cover"
                              />
                              <div className="ml-3">
                                <p className="font-medium text-gray-800">{moderator.fullName}</p>
                    ...
                    <div className="md:w-2/3">
                      {selectedModerator ? (
                        <div className="flex flex-col h-full">
                          {/* Chat Header */}
                          <div className="flex items-center p-3 border-b border-gray-200 bg-gray-50 rounded-t-lg">
                            <img
                              src={selectedModerator.avatar || generateUniqueAvatar(selectedModerator)}
                              alt={selectedModerator.fullName}
                              className="w-10 h-10 rounded-full object-cover"
                            />
                            <div className="ml-3">
                              <p className="font-medium text-gray-800">{selectedModerator.fullName}</p>
                              <p className="text-sm text-gray-600">Moderator</p>
                            </div>
                          </div>
                          
                          {/* Messages Container */}
                          <div className="flex-1 overflow-y-auto p-4 bg-gray-50 rounded-b-lg h-96">
                            {[...moderatorMessages]
                              .filter(msg => 
                                (msg.from._id === selectedModerator._id && msg.to._id === JSON.parse(localStorage.getItem('tathya_user'))._id) ||
                                (msg.from._id === JSON.parse(localStorage.getItem('tathya_user'))._id && msg.to._id === selectedModerator._id)
                              )
                              ...
                              .map((msg) => (
                                <div
                                  key={msg._id}
                                  className={`flex mb-4 ${
                                    msg.from._id === JSON.parse(localStorage.getItem('tathya_user'))._id
                                      ? 'justify-end'
                                      : 'justify-start'
                                  }`}
                                >
                          ...
                          <div className="mt-4 flex">
                            ...
                            <motion.button
                              ...
                              className={`px-6 py-3 rounded-r-lg font-medium text-white transition-all duration-300 ${
                        ...
                        <div className="flex items-center justify-center h-96 bg-gray-50 rounded-lg">
                          <div className="text-center">
                            ...
                            <p className="text-gray-500">Select a moderator to start chatting</p>
                          </div>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="text-center py-8">
                    <i className="fas fa-user-shield text-gray-400 text-3xl mb-3"></i>
                    <p className="text-gray-500">No moderators available at the moment.</p>
                  </div>
                )}
              </div>
            </div>
          ...
          {/* Contact Queries section after Moderator Messages section */}
          {activeMenu === 'contact-queries' && (
            <div className="contact-queries-section animate-fade-in">
              <h2 className="text-2xl font-bold text-gray-800 mb-6">My Contact Queries</h2>
              <div className="bg-white p-6 rounded-lg shadow-md">
                <p className="text-gray-600 mb-4">Here you can view all your contact form submissions and responses from moderators.</p>
                ...
                <div className="queries-list space-y-4">
                  {loadingQueries ? (
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-500">You haven't submitted any contact queries yet.</p>
                      <motion.button
                        whileHover={{ scale: 1.03 }}
                        whileTap={{ scale: 0.97 }}
                        onClick={() => navigate('/contact')}
                        className="mt-4 contact-moderator-btn bg-gradient-to-r from-blue-600 to-blue-700 text-white py-2 px-4 rounded-md transition-all duration-300 inline-flex items-center shadow-md hover:shadow-lg"
                      >
                        <i className="fas fa-paper-plane mr-2"></i> Submit a Query
                      </motion.button>
                    </div>
                  ) : (
                    [...contactQueries]
                      ...
                      .map((query) => {
                        // Check if this is a moderator response
                        const isResponse = query.from.role === 'moderator' && query.content.includes('Response to your contact form submission');
                        ...
                        if (isResponse) {
                          // This is a moderator response
                          const subjectLine = lines.find(line => line.includes('Original Subject:'));
                          subject = subjectLine ? subjectLine.replace('Original Subject: ', '') : 'Response to Contact Form';
                          
                          // Extract response content (everything after "Response:")
                          const responseStartIndex = lines.findIndex(line => line.trim() === 'Response:');
                          messageContent = responseStartIndex > -1 
                            ? lines.slice(responseStartIndex + 1).join('\n').trim() 
                            : query.content;
                        } else {
                          // This is user's contact form submission
                          const subjectLine = lines.find(line => line.startsWith('Subject:'));
                          subject = subjectLine ? subjectLine.replace('Subject: ', '') : 'Contact Form Submission';
                        ...
                        return (
                          <div key={query._id} className="query-card border border-gray-200 rounded-lg p-4 bg-gray-50">
                            <div className="flex justify-between items-start">
                              <div>
                                <h4 className="font-medium text-gray-800">{isResponse ? `Response: ${subject}` : subject}</h4>
                                <p className="text-sm text-gray-600">{isResponse ? 'Response from moderator' : 'Submitted on'}: {formatTimestamp(query.timestamp)}</p>
                              </div>
                              <span className={`px-2 py-1 text-xs font-medium rounded-full ${isResponse ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'}`}>
                                {isResponse ? 'Response' : (query.read ? 'Read' : 'Unread')}
                              </span>
                            </div>
                            <div className="mt-3">
                              <p className="text-gray-700">{messageContent.substring(0, 150)}{messageContent.length > 150 ? '...' : ''}</p>
                            </div>
                ...
                <div className="mt-6">
                  ...
                  >
                    <i className="fas fa-paper-plane mr-2"></i> Submit New Query
                  </motion.button>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl h-3/4 flex flex-col">
            ...
            <div className="flex-grow p-4 overflow-auto">
              ...
              {docPreview.type === 'application/pdf' && (
                <embed src={docPreview.url} type="application/pdf" width="100%" height="100%" className="min-h-[600px]" />
              )}
              ...
              {!docPreview.type.includes('image') && docPreview.type !== 'application/pdf' && !docPreview.type.includes('text') && !docPreview.type.includes('json') && !docPreview.type.includes('xml') && (
                <div className="flex flex-col items-center justify-center h-full">
                  <p className="text-gray-600 mb-4">No preview available for this document type.</p>
                  <a 
                    href={docPreview.url} 
