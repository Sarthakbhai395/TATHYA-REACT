...
const CommunityDashboard = () => {
  ...
  const { communityId } = useParams(); // Get community ID from URL params if needed
  ...
  const [posts, setPosts] = useState([
    {
      id: 1,
      author: { name: 'Anonymous Student', avatar: 'https://randomuser.me/api/portraits/men/32.jpg' },
      content: 'Feeling overwhelmed with the upcoming exams. My professors are setting unrealistic expectations. Any tips on managing stress?',
      ...
      comments: [
        {
          id: 101,
          author: { name: 'Protected Identity', avatar: 'https://randomuser.me/api/portraits/women/44.jpg' },
          content: 'I totally understand your stress. Try breaking down the syllabus into smaller chunks and setting daily goals.',
          ...
          replies: [
            {
              id: 201,
              author: { name: 'Another User', avatar: 'https://randomuser.me/api/portraits/men/22.jpg' },
              content: 'That\'s a great suggestion!',
        ...
        {
          id: 102,
          author: { name: 'Verified Member', avatar: 'https://randomuser.me/api/portraits/women/68.jpg' },
          content: 'Remember, it\'s okay to ask for help. Reach out to your classmates or the community support group.',
    ...
    {
      id: 2,
      author: { name: 'Student Voice', avatar: 'https://randomuser.me/api/portraits/men/45.jpg' },
      content: 'Just wanted to share that I finally got my harassment report resolved thanks to TATHYA\'s guidance. Feeling relieved!',
      images: ['https://images.unsplash.com/photo-1522202176988-66273c2fd55f?q=80&w=387&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'], // Example image
      videos: [], // No videos for this post
      timestamp: '1 day ago',
      likes: 156,
      likedByCurrentUser: true,
      comments: [
        {
          id: 103,
          author: { name: 'Community Moderator', avatar: 'https://images.unsplash.com/photo-1593085650650-1a0d3e2d0bca?q=80&w=387&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D' },
          content: 'Congratulations on taking this step and getting resolution! Your courage inspires others.',
    ...
    {
      id: 3,
      author: { name: 'Future Leader', avatar: 'https://randomuser.me/api/portraits/women/33.jpg' },
      content: 'Working on my resume with TATHYA\'s builder. It\'s amazing how much easier it makes the process!',
      images: [], // No images for this post
      videos: ['https://sample-videos.com/video123/mp4/720/big_buck_bunny_720p_1mb.mp4'], // Example video URL (replace with actual video)
  ...
  useEffect(() => {
    if (location && location.state && location.state.community) {
      const passed = location.state.community;
      // Map basic community fields
      setCommunityData(prev => ({
        ...prev,
        id: passed.id || prev.id,
        name: passed.name || prev.name,
        region: passed.region || prev.region,
        members: passed.members || prev.members,
        description: passed.description || prev.description,
        image: passed.image || prev.image,
        tags: passed.tags || prev.tags,
        isJoined: passed.isJoined !== undefined ? passed.isJoined : prev.isJoined,
        icon: passed.icon || prev.icon,
      }));
      // Map posts from AddToCommunity format to CommunityDashboard expected shape
      if (Array.isArray(passed.posts)) {
        const mappedPosts = passed.posts.map(p => ({
          id: p.id || Date.now(),
          author: { name: p.author || 'Anonymous', avatar: 'https://randomuser.me/api/portraits/lego/1.jpg' },
          ...
          comments: (p.comments || []).map(c => ({
            id: c.id || Date.now(),
            author: { name: c.author || 'Anonymous', avatar: 'https://randomuser.me/api/portraits/lego/2.jpg' },
            content: c.content || '',
            ...
            replies: (c.replies || []).map(r => ({
              id: r.id || Date.now(),
              author: { name: r.author || 'Anonymous', avatar: 'https://randomuser.me/api/portraits/lego/3.jpg' },
              content: r.content || '',
  ...
  const handleFileSelect = (event, type) => {
    ...
    if (type === 'image') {
      ...
      setNewPostImages(prev => [...prev, ...imageFiles.map(file => URL.createObjectURL(file))]);
    } else if (type === 'video') {
      // Filter only video files
      const videoFiles = files.filter(file => file.type.startsWith('video/'));
      setNewPostVideos(prev => [...prev, ...videoFiles.map(file => URL.createObjectURL(file))]);
    }
  ...
  const handleRemoveMedia = (index, type) => {
    if (type === 'image') {
      ...
      URL.revokeObjectURL(newVideos[index]); // Free memory
  ...
  const handlePublishPost = () => {
    ...
    const newPost = {
      id: newPostId,
      author: { name: 'You', avatar: 'https://randomuser.me/api/portraits/men/32.jpg' }, // Use current user's data
      content: newPostContent,
      images: newPostImages, // Store image URLs
      videos: newPostVideos, // Store video URLs
      timestamp: 'Just now',
      likes: 0,
      likedByCurrentUser: false,
      comments: [],
  ...
  const handleAddComment = (postId) => {
    ...
    const newCommentId = posts.flatMap(p => p.comments).length > 0
      ? Math.max(...posts.flatMap(p => p.comments).map(c => c.id)) + 1
      : 1;
    const newCommentObj = {
      id: newCommentId,
      author: { name: 'You', avatar: 'https://randomuser.me/api/portraits/men/32.jpg' }, // Use current user's data
  ...
  // --- Handle Add Reply ---
  const handleAddReply = (commentId, postId) => {
    const replyText = newReply.content?.trim();
    if (!replyText) return;
    const newReplyId = posts.flatMap(p => p.comments.flatMap(c => c.replies)).length > 0
      ? Math.max(...posts.flatMap(p => p.comments.flatMap(c => c.replies)).map(r => r.id)) + 1
      : 1;
    const newReplyObj = {
      id: newReplyId,
      author: { name: 'You', avatar: 'https://randomuser.me/api/portraits/men/32.jpg' }, // Use current user's data
                ...
                <div className="mb-4">
                  ...
                  {(newPostImages.length > 0 || newPostVideos.length > 0) && (
                    <div className="mt-4">
                      ...
                      <div className="flex flex-wrap gap-3">
                        {/* Image Previews */}
                        {newPostImages.map((imgUrl, index) => (
